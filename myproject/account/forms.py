...
class SignUpForm(UserCreationForm):  # 회원가입 기본 폼 상속 (패스워드, 패스워드확인)
    ...
    def save(self, commit=True, *args, **kwargs):
        # 본인의 부모(UserCreationForm)를 호출하여 저장 + 일단 보류
...
class IncumbentSignUpForm(SignUpForm):  # 일반 회원가입 폼 상속
    ...
    def clean(self, *args, **kwargs):
        ...
        if email.split('@')[1] != 'naver.com':  # 이메일의 @ 뒷부분 (추후 변경)
            # forms.ValidationError 보내면, views의 form_invalid()에서 form.non_field_errors()로
            raise forms.ValidationError('롯데 이메일 계정이 아닙니다.')
        if line is None:
            # forms.ValidationError 보내면, views의 form_invalid()에서 form.non_field_errors()로
            raise forms.ValidationError('계열을 입력하세요.')
    ...
    def save(self, commit=True, *args, **kwargs):
        # 본인의 부모(UserCreationForm)를 호출하여 저장 + 일단 보류
        user = super(IncumbentSignUpForm, self).save(
